...
\tikzstyle{line}=[-, thick]
\tikzstyle{class}=[rectangle, draw=black, text centered, anchor=north, text=black, text width=3cm, shading=axis,
    bottom color=black!20,top color=white,shading angle=45]
\tikzstyle{interface}=[rectangle, draw=black, text centered, anchor=north, text=black, text width=3cm, shading=axis,
    bottom color=blue!20,top color=white,shading angle=45]

        
\begin{center}
\begin{tikzpicture}[node distance=2cm]
    \node (ITransaction) [interface, rectangle split, rectangle split parts=2]
        {
            $<<$interface$>>$ \textbf{ITransaction}
            \nodepart{second}methods()
        };
    % \node (AuxNode01) [text width=4cm, below=of ITransaction] {};
    \node (AbstractTransaction) [class, rectangle split, rectangle split parts=2, below=of ITransaction]
        {
            \textbf{AbstractTransaction}
            \nodepart{second}nil
        };

    \node (NormalTransaction) [class, rectangle split, rectangle split parts=2, left=of AbstractTransaction]
        {
            \textbf{Transaction}
            \nodepart{second}nil
        };
    \node (RecurringTransaction) [class, rectangle split, rectangle split parts=2, right=of AbstractTransaction]
        {
            \textbf{RecurringTransaction}
            \nodepart{second}nil
        };
